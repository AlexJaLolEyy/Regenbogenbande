...
async function fetchPictures(
    ...
    const response = await fetch(`/api/pictures?${params.toString()}`, { signal });
    if (!response.ok) throw new Error('Failed to fetch pictures');
    return response.json();
}

export function usePictures(filters: FilterState, initialData?: PictureListItem[]) {
    ...
    return useInfiniteQuery({
        queryKey: queryKeys.pictures.list(filters),
        queryFn: ({ pageParam, signal }) => fetchPictures(filters, pageParam, signal),
        initialPageParam: 1,
        getNextPageParam: (lastPage, allPages) => {
            return lastPage.length === PICTURES_PER_PAGE ? allPages.length + 1 : undefined;
        },
