...
export default function QuoteList({ initialQuotes, categories }: { initialQuotes: QuoteListItem[], categories: { id: string; name: string, iconUrl?: string | null }[] }) {
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
            ...
            <div className="max-w-400 mx-auto space-y-8">
              <div className="flex justify-between items-center mb-8 pl-1">
                <div>
                  <h1 className="text-3xl font-bold text-white text-glow">Archived Quotes</h1>
                  <p className="text-white/40 text-sm">Refined Gallery</p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-sm text-white/50 border border-white/10 px-3 py-1 rounded-full bg-white/5 backdrop-blur-md">
              ...
              {loading && quotes.length === 0 ? (
                <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 2xl:columns-5 gap-6 space-y-6">
                  {Array.from({ length: 15 }).map((_, i) => (
                    <div key={i} className="break-inside-avoid bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md animate-pulse">
